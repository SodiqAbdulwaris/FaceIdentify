...
constraint. Rows come from the shared `build` factory (tests/factories/models.py).
"""
...
from backend.app.jobs.models import JobPriority, JobState, JobType, ProgressMode
...
from backend.app.runtime.models import ModelExport
...
from tests.factories.models import SHA, ModelFactory
...
def test_utc_datetime_round_trips_and_normalises_offsets(build: ModelFactory) -> None:
...
def test_utc_datetime_rejects_naive_values(build: ModelFactory) -> None:
...
def test_spec_defined_value_sets_reject_unknown_literals(
    build: ModelFactory, row: str, column: str
) -> None:
...
def test_segment_and_checkpoint_value_sets(build: ModelFactory) -> None:
    run = build.run()
    rejected(
        build.session, lambda: build.segment(run, 0, "PAUSED"), check("ck_execution_segments_state")
    )
    run = build.run()
    rejected(
        build.session,
...
def test_segment_end_reason_is_optional_but_constrained(build: ModelFactory) -> None:
    run = build.run()
...
def test_managed_artifact_requires_storage_key_and_no_external_path(build: ModelFactory) -> None:
    rejected(
        build.session, lambda: build.artifact(storage_key=None), check("ck_artifacts_location")
    )
    rejected(
        build.session,
...
def test_referenced_artifact_requires_external_path_and_no_storage_key(build: ModelFactory) -> None:
...
def test_available_managed_artifact_must_be_verified(build: ModelFactory) -> None:
    pending = build.artifact(state="PENDING", sha256=None, size_bytes=None)
    assert pending.sha256 is None
    rejected(
        build.session,
        lambda: build.artifact(sha256=None),
...
def test_artifact_hash_must_be_32_bytes(build: ModelFactory) -> None:
    rejected(
        build.session, lambda: build.artifact(sha256=b"short"), check("ck_artifacts_sha256_length")
    )


def test_managed_storage_keys_are_unique(build: ModelFactory) -> None:
    build.artifact(storage_key="originals/one")
    rejected(
        build.session,
        lambda: build.artifact(storage_key="originals/one"),
        unique("artifacts.storage_key"),
    )


def test_source_display_name_must_not_be_blank(build: ModelFactory) -> None:
    rejected(
        build.session,
        lambda: build.source(display_name="   "),
        check("ck_sources_display_name_not_empty"),
    )


def test_source_media_facts_must_not_be_negative(build: ModelFactory) -> None:
    rejected(build.session, lambda: build.source(width=-1), check("ck_sources_width_non_negative"))


def test_source_original_artifact_cannot_be_deleted(build: ModelFactory) -> None:
    source = build.source()
    rejected(
        build.session,
...
def test_deleting_a_thumbnail_artifact_clears_the_source_reference(build: ModelFactory) -> None:
...
def test_each_snapshot_belongs_to_exactly_one_run(build: ModelFactory) -> None:
    run = build.run()
    rejected(
        build.session,
...
def test_only_one_running_segment_per_run(build: ModelFactory) -> None:
    run = build.run()
...
def test_segment_ordinals_are_unique_within_a_run(build: ModelFactory) -> None:
    run = build.run()
    build.segment(run, 0, "COMPLETED")
    rejected(
        build.session,
...
def test_only_one_valid_final_checkpoint_per_run(build: ModelFactory) -> None:
    run = build.run()
...
def test_run_cannot_point_at_a_missing_checkpoint(build: ModelFactory) -> None:
    rejected(build.session, lambda: build.run(current_checkpoint_id=uuid.uuid4()), "FOREIGN KEY")


# --- jobs, catalog, settings -----------------------------------------------------------------


def test_job_progress_cannot_be_negative(build: ModelFactory) -> None:
    rejected(
        build.session,
        lambda: build.job(progress_completed=-1),
        check("ck_jobs_completed_non_negative"),
    )
    rejected(
        build.session, lambda: build.job(progress_total=-1), check("ck_jobs_total_non_negative")
    )


def test_job_progress_cannot_exceed_total(build: ModelFactory) -> None:
...
def test_job_retry_links_to_an_existing_previous_job(build: ModelFactory) -> None:
    ...
    rejected(build.session, lambda: build.job(previous_job_id=uuid.uuid4()), "FOREIGN KEY")


def test_representation_space_rules(build: ModelFactory) -> None:
    rejected(
        build.session,
...
def test_remaining_integrity_checks(build: ModelFactory) -> None:
    ...
    rejected(build.session, lambda: build.source(revision=0), check("ck_sources_revision_positive"))
    rejected(
        build.session, lambda: build.run(revision=0), check("ck_processing_runs_revision_positive")
    )
    rejected(
        build.session, lambda: build.job(attempt_number=0), check("ck_jobs_attempt_number_positive")
    )
    rejected(
        build.session,
        lambda: build.artifact(size_bytes=-1),
        check("ck_artifacts_size_non_negative"),
    )
    rejected(
        build.session,
        lambda: build.add(
